...
/**
 * Redirect strategy for http client that allows following redirects (HTTP 301, 302, 307) for POST
 * messages after validating the redirect location is "safe".
 *
 * <p>This class does not limit redirect hops; DefaultHttpClient limits redirect hops to 100 (see
...
public class AutodiscoverRedirectStrategy extends LaxRedirectStrategy {
   ...
   * Behavior of <code>DefaultRedirectStrategy</code> is to use a GET for the redirect (though
   * against spec this is the de-facto standard, see
   * http://www.mail-archive.com/httpclient-users@hc.apache.org/msg06327.html and
   * http://www.alanflavell.org.uk/www/post-redirect.html).
   *
   * <p>For our application, we want to follow the redirect for a 302 as long as it is to a safe
   * location and have the redirect be a POST.
   *
   * <p>This code is modified from http-components' http-client 4.2.5. Since we only use POST the
   * code for the other HTTP methods has been removed to simplify this method.
   *
  ...
  public HttpUriRequest getRedirect(
    ...
    if (!matchesPatternSet(uri, uriRequirementPatterns) && uriRequirementPatterns.size() > 0) {
      log.warn(
          "Not following to URI {} - URI does not match a required URI pattern", uri.toString());
