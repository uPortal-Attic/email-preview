 ...
 *
 * <p>NOTE: With Javamail, uuids are not globally-unique within the user's mail store as they are
 * with Exchange for instance. With Javamail the messageIds are index numbers within the mailbox
 * store (which is really per-folder). Thus, many mail folders for a user may have a messageId=1.
 * This does not present many problems as long as we insure we are only dealing with one folder at a
 * time.
 *
 * @author awills
...
public final class JavamailAccountDaoImpl implements IMailAccountDao {
  ...
  @Autowired private ICredentialsProvider credentialsProvider;
  ...
  public AccountSummary fetchAccountSummaryFromStore(
    ...
    try {
      ...
      if (log.isDebugEnabled()) {
        ...
        log.debug(
            "Finished looking up email messages. Inbox size: "
                ...
                + elapsedTime
                + "ms "
                + " Time per message in inbox: "
                + (inbox.getMessageCount() == 0 ? 0 : (elapsedTime / inbox.getMessageCount()))
                + "ms"
  ...
  private EmailMessage wrapMessage(Message msg, boolean populateContent, Session session)
    ...
    if (populateContent) {
      ...
      try {
        ...
        try {
          ...
          msgContent =
              new EmailMessageContent("UNABLE TO READ MESSAGE BODY: " + t.getMessage(), false);
        }
      }
    ...
    try {
      ...
      log.debug(
          "Message content unavailable (digitally signed?);  "
  ...
  private List<EmailMessage> getEmailMessages(
      Folder mailFolder, int pageStart, int messageCount, Session session)
      throws MessagingException, IOException, ScanException, PolicyException {
    ...
    Message[] messages =
        totalMessageCount != 0 ? mailFolder.getMessages(start, end) : new Message[0];
    ...
    if (log.isDebugEnabled()) {
      log.debug(
          "Time elapsed while fetching message headers; {}ms",
  ...
  /**
   * Determine if the supplied MIME type represents HTML content. This implementation assumes that
   * the inclusion of the string "text/html" in a mime-type indicates HTML content.
   *
   * @param mimeType
   * @return
   */
