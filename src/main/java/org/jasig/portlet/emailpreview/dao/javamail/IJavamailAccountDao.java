...
/**
 * This interface exists to work with the caching annotations. This interface is package-private by
 * intent.
 *
 * @author awills
 */
interface IJavamailAccountDao {
  ...
  /**
   * Performs the heavy-lifting of {@link IEmailAccountService} but in a way that exposes Javamail
   * implementation details. Caching annotations can be wrapped around these methods.
   *
   * @param storeConfig
   * @param auth
   ...
   * @param max Maximum size of the collection of messages in the returned {@link
   *     org.jasig.portlet.emailpreview.AccountSummary} object
  ...
  EmailMessage wrapMessage(Message msg, boolean populateContent, Session session)
