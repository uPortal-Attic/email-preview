...
public class AjaxUpdateInboxFolderController extends BaseEmailController {
  ...
  public ModelAndView inboxFolder(ResourceRequest req, ResourceResponse res)
    ...
    String selectedFolder =
        prefs.getValue(
            EmailAccountSummaryController.INBOX_NAME_PREFERENCE, INBOX_DEFAULT_VALUE_PREF);
    String authenticationServiceKey =
        prefs.getValue(AUTH_SERVICE_PREFERENCE, AUTH_SERVICE_DEFAULT_VALUE_PREFERENCE);
    ...
    if (AUTH_SERVICE_TEST_PREFERENCE.equals(authenticationServiceKey)) {
      ...
      for (Folder folderName : accountDao.getAllUserInboxFolders(req)) {
        if ((folderName.getType() & javax.mail.Folder.HOLDS_MESSAGES) != 0) {
          ...
          if (folderName.getUnreadMessageCount() != 0
              && (!selectedFolder.equals(folderName.toString()))) {
            unreadMsgCount =
                " ("
                    .concat(String.valueOf(folderName.getUnreadMessageCount()))
                    .concat(")")
                    .concat(UNREAD_MSG_FLAG);
          }
