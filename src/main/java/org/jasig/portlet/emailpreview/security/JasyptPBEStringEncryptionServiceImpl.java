...
public class JasyptPBEStringEncryptionServiceImpl implements IStringEncryptionService {
  ...
  public String encrypt(final String plaintext) {
    try {
      ...
      throw new StringEncryptionException(
          "Encryption error. Verify an encryption password"
              + " is configured in configuration.properties",
          e);
    }
  }
  ...
  private String decrypt(final String cryptotext, boolean logError) {
    try {
      ...
      throw new StringEncryptionException(
          "Decryption error. Was encryption password" + " changed in the configuration.properties?",
          e);
    } catch (EncryptionOperationNotPossibleException e) {
      if (logError) {
        logger.warn(
            "Decryption failed.  This suggests the salt key in "
