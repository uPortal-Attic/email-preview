...
public class ResourceUtils implements InitializingBean, ResourceLoaderAware {
	...
	public void afterPropertiesSet() throws Exception {
		...
		try {
			Resource iconsFolder = rl.getResource("images/icons");
			...
			for(File icon: files) {
				...
				icons.put(iconName.substring(0, iconName.length()-4), "/email-preview/images/icons/".concat(iconName));
			}
	...
	private String getIconFromMime(String mime) {
		...
		if(icons.containsKey(mime)) 
			...
			return "/email-preview/images/icons/unknown.png";
	}
