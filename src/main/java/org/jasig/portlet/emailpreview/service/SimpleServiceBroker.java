...
public class SimpleServiceBroker implements IServiceBroker {
  ...
  protected static final List<String> RESERVED_PROPERTIES =
      Arrays.asList(
          MailPreferences.HOST.getKey(),
          MailPreferences.PORT.getKey(),
          MailPreferences.INBOX_NAME.getKey(),
          MailPreferences.PROTOCOL.getKey(),
          MailPreferences.TIMEOUT.getKey(),
          MailPreferences.CONNECTION_TIMEOUT.getKey(),
          MailPreferences.LINK_SERVICE_KEY.getKey(),
          MailPreferences.AUTHENTICATION_SERVICE_KEY.getKey(),
          MailPreferences.ALLOWABLE_AUTHENTICATION_SERVICE_KEYS.getKey(),
          MailPreferences.USERNAME_SUFFIX.getKey(),
          MailPreferences.ALLOW_RENDERING_EMAIL_CONTENT.getKey(),
          MailPreferences.EWS_USE_MAIL_ATTRIBUTE.getKey(),
          MailPreferences.EXCHANGE_AUTODISCOVER.getKey(),
  ...
  public MailStoreConfiguration getConfiguration(PortletRequest request) {
    ...
    String[] authServiceKeys =
        prefs.getValues(
            MailPreferences.ALLOWABLE_AUTHENTICATION_SERVICE_KEYS.getKey(), new String[0]);
    ...
    config.setMarkMessagesAsRead(
        Boolean.valueOf(prefs.getValue(MailPreferences.MARK_MESSAGES_AS_READ.getKey(), "true")));
    ...
    try {
      int connectionTimeout =
          Integer.parseInt(prefs.getValue(MailPreferences.CONNECTION_TIMEOUT.getKey(), "25000"));
        ...
        if (key.startsWith("mail.")) {
          ...
          if (param != null && param.isEncryptionRequired()) {
            if (stringEncryptionService == null) {
              final String msg =
                  "The following setting requires "
                      + "encryption but the 'stringEncryptionService' "
                      + "bean is not configured:  "
                      + entry.getKey();
  ...
  public void saveConfiguration(ActionRequest request, MailStoreConfiguration config) {
    ...
    try {
      ...
      if (!prefs.isReadOnly(MailPreferences.LINK_SERVICE_KEY.getKey())) {
        prefs.setValue(
            MailPreferences.LINK_SERVICE_KEY.getKey(), String.valueOf(config.getLinkServiceKey()));
      }
