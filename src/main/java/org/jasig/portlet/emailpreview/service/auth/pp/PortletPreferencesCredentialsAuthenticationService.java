...
public class PortletPreferencesCredentialsAuthenticationService
  ...
  private static final String ACCOUNT_NAME_ATTRIBUTE_KEY =
      "PortletPreferencesCredentialsAuthenticationService.ACCOUNT_NAME_ATTRIBUTE";
  private static final String ACCOUNT_NAME_ATTRIBUTE_LABEL =
      "If specified, use this user attribute as the mail account name (users won't be able to edit it)";
  private static final ConfigurationParameter ACCOUNT_NAME_ATTRIBUTE =
  ...
  public PortletPreferencesCredentialsAuthenticationService() {
    ...
    setAdminParameters(
        Collections.<ConfigurationParameter>singletonList(
            PortletPreferencesCredentialsAuthenticationService.ACCOUNT_NAME_ATTRIBUTE));
  ...
  public Credentials getCredentials(PortletRequest req, MailStoreConfiguration config) {
    String ntlmDomain =
        config.getAdditionalProperties().get(MailPreferences.EXCHANGE_DOMAIN.getKey());
    String password = config.getAdditionalProperties().get(MailPreferences.PASSWORD.getKey());
  ...
  public String getMailAccountName(PortletRequest req, MailStoreConfiguration config) {
     ...
     */
    final String accountNameAttribute =
        config.getAdditionalProperties().get(ACCOUNT_NAME_ATTRIBUTE_KEY);
