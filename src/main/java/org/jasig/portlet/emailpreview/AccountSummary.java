...
/**
 * Encapsulates basic information about the email INBOX. Typically sent to the browser via AJAX.
 *
...
public final class AccountSummary {
  ...
  /**
   * Indicates the account fetch did not succeed and provides the cause. Previously we would
   * communicate this fact by throwing an exception, but we learned that (1) the caching API we use
   * writes these exceptions to Catalina.out, and (2) occurances of {@link
   * MailAuthenticationException} are very common, and the logs were being flooded with them.
   *
   * @param errorCause
   */
  ...
  /**
   * Indicates if this object contains valid account details. Otherwise, it will contain an error
   * payload.
   *
   * @return False if this object represents an error condition instead of an account summary
   */
  ...
  /**
   * Returns a list that contains the emails bound by <code>messagesStart</code> and <code>
   * messagesCode</code>.
   *
   * @return A <code>List<EmailMessage></code> containing information about emails in the user's
   *     inbox
   */
  ...
  /**
   * Returns the number of messages requested for the Messages list. The actual size of the list may
   * be lower.
   *
   * @return
   */
